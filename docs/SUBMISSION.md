...
| **Two-stage trust pipeline** | Stage 1 (Gemini): extract claims + category scores. Stage 2 (Gemini): match each claim to fetched source excerpts. Separating extraction from matching improves structured output reliability |
...
| **5-agent code review (ADK)** | Secondary product: Google ADK `ParallelAgent` runs five specialists, then a coordinator synthesizes a developer-actionable report — same Gemini stack, different orchestration pattern |
...
| **Google ADK** | `ParallelAgent` + `SequentialAgent` for 5-agent code review | Built-in multi-agent orchestration with typed agent outputs — cleaner than hand-rolled coordinator logic for the code-review path |
...
| --- | --- |
| **Technical depth (35%)** | Multi-stage trust pipeline, source fetching, claim-to-excerpt matching, weighted scoring, ADK parallel agent graph, 44+ unit tests |
| **Google technology (20%)** | Gemini structured output throughout; ADK for code review; Vertex + Cloud Run deployment path; Chrome extension integration |
...
1. **Trust eval harness** — Labeled claim-support samples (like `eval/` for code review) to measure matching accuracy
2. **ADK for trust pipeline** — Model claim extraction → source check → scoring as an ADK graph for a unified Google story
3. **More platforms** — Gemini app, Perplexity, Claude via shared content-script adapters
4. **Caching + latency** — Cache fetched sources; parallelize Gemini calls where safe
5. **History dashboard** — Save past analyses for students and researchers
6. **Stronger source fetching** — Readability extraction, paywall detection, official-domain allowlists per topic
